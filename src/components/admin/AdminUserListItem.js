        ...
        if (user !== null && user !== undefined) {
            return (
                <tr  key={index + 'klcwnoasdsa'}>
                    ...
                    <td>
                        {isLocalUsers ? (<Input placeholder={user.totalDays+"/"+user.vacationDays} value={this.state.vacationDays} onChange={(e)=>this.onVacationDaysChange(e)} min={user.totalDays} max={1000} type="number" step="1" />):"-"}
                        {isLocalUsers && this.state.vacationDays !== null ? (
                            <div>
                                <Badge onClick={()=>this.changeVacationDays()} style={{ marginLeft: '5px', cursor: "pointer" }} color="info">Update</Badge>
                                <Badge onClick={()=>{this.setState({vacationDays: null})}} style={{ marginLeft: '5px', cursor: "pointer" }} color="danger">X</Badge>
                            </div>
                        ) : null}
                    </td>
                </tr>
